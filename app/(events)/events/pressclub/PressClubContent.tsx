  ...
  return (
    <DownloadQueueProvider eventId={event.id}>
      <PhoneUnlockProvider websiteLabel={WEBSITE_LABEL}>
        <PressClubInner event={event} initialPhotos={initialPhotos} totalCount={totalCount} hasMore={hasMore} />
      </PhoneUnlockProvider>
    </DownloadQueueProvider>
  );
}

function PressClubInner({ event, initialPhotos, totalCount, hasMore }: PressClubContentProps) {
  ...
  const { trackEvent } = useNessusTracking("2016 Night at Press Club", WEBSITE_LABEL);
