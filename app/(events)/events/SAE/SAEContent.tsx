...
const WEBSITE_LABEL = "sae-house-party";

interface SAEContentProps {
  event: GalleryEvent;
...
export function SAEContent({ event, initialPhotos, totalCount, hasMore }: SAEContentProps) {
  return (
    <DownloadQueueProvider eventId={event.id}>
      <PhoneUnlockProvider websiteLabel={WEBSITE_LABEL}>
        <SAEInner event={event} initialPhotos={initialPhotos} totalCount={totalCount} hasMore={hasMore} />
      </PhoneUnlockProvider>
    </DownloadQueueProvider>
  );
}

function SAEInner({ event, initialPhotos, totalCount, hasMore }: SAEContentProps) {
  const [bladeOpen, setBladeOpen] = useState(false);
  const [downloadBladeOpen, setDownloadBladeOpen] = useState(false);
  const { trackEvent } = useNessusTracking("SAE House Party", WEBSITE_LABEL);
